# main.py — the only file you need to edit to run experiments
...
CSV_FILE = '../data/scope_0.csv'   # only used when MODE = 'real'

# Which estimators to compare — comment out any you don't want
ACTIVE_ESTIMATORS = [
    'zero_crossing',
    'argmax',
    'cma',
...
def run_synthetic():
    ...
    for velocity, noise, label in SYNTHETIC_TESTS:
        ...
        plot_estimator_comparison(
            ...
            filename=f'comparison_{safe_label}.png'
        )
        ...
        plot_pipeline_stages(
            results[detail_est],
            true_freq=true_freq,
            filename=f'pipeline_{safe_label}.png'
        )
...
def run_real():
    ...
    plot_estimator_comparison(
        ...
        filename='comparison_real.png'
    )

    detail_est = 'xca' if 'xca' in ACTIVE_ESTIMATORS else ACTIVE_ESTIMATORS[0]
    plot_pipeline_stages(results[detail_est], filename='pipeline_real.png')
